...
EOF
    ...
    echo "  [1] Pre-check              → Root/OS/Network verification"
    echo "  [2] Cleanup                → Remove old files and users"
    echo "  [3] Kill processes/ports   → Free required ports (except SSH)"
    echo "  [4] Create user            → Setup taxi user with permissions"
    echo "  [5] Install packages       → APT and npm dependencies"
    echo "  [6] Configure firewall     → UFW rules and port opening"
    echo "  [7] Validation             → Start services and verify ports"
    echo "  [8] Log and summary        → Create installation report"
    echo ""
    echo "Summary:"
