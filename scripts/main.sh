...
EOF
    ...
    echo "Installation Flow Summary:"
    echo "  [1] Pre-check ────────────→ Root/OS/Network verified"
    echo "  [2] Cleanup ───────────────→ Old files/users/processes removed"
    echo "  [3] Kill processes/ports ──→ All ports cleared and free"
    echo "  [4] Create taxi user ─────→ User created with home structure"
    echo "  [5] Install packages ─────→ Dependencies, Docker, npm installed"
    echo "  [6] Firewall config ──────→ UFW enabled with ports open"
    echo "  [7] Validation ────────────→ All services verified and running"
    echo "  [8] Log and summary ──────→ Installation logged and reported"
    echo ""
    echo "Services:"
    echo "  ✓ $SERVICES_OK/4 Taxi services responding"
    echo "  ✓ Docker: Running with all required containers"
